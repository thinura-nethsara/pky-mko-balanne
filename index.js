...
if (!fs.existsSync(df)) {
  if (config.SESSION_ID) {
    const sessdata = config.SESSION_ID.replace("DARKLEX-MD&", "");
...
const responsee = await axios.get('https://darklexjson.vercel.app/darklex-md%20database.json');
    ...
    if (connection === 'close') {
        ...
        setTimeout(async () => {
            try {
                ...
                let captionText = '✅ DARKLEX connected successfully!';
                try {
                    const response = await axios.get('https://darklexjson.vercel.app/darklex-md%20database.json');
                ...
                await conn.sendMessage(DEFAULT_OWNER_JID, {
                    image: { url: 'https://i.ibb.co/VpzjvxBh/temp-image.jpg' },
                    caption: captionText
                });
...
`;
     ...
     let joinlink2 = await fetchJson('https://darklexjson.vercel.app/darklex-md%20database.json');
...
await connectdb()
await updb()		
console.log('DARKLEX-MD CONNECTED ✅')
...
const ownerdataa = (await axios.get('https://darklexjson.vercel.app/darklex-md%20database.json')).data;
...
console.log("DARKLEX UPDATES CHANAL FOLLOW ✅")
}	 
...
const ownerdata = (await axios.get(
  'https://darklexjson.vercel.app/darklex-md%20database.json'
)).data

// ❌ config.footer
// ✅ config.FOOTER  (case fix)
config.FOOTER = ownerdata?.FOOTER || '•ᴅᴀʀᴋʟᴇx-ᴍᴅ•'
...
// "@s.whatsapp.net" format එකට convert කරලා check
...
const recc = (await axios.get('https://darklexjson.vercel.app/darklex-md%20database.json')).data
...
await conn.newsletterReactMessage(`120363422074013001@newsletter`, id, randomEmoji);
...
if ( isBanUser ) {
	...
	return await conn.sendMessage(from, { text: "*You are banned by DARKLEX TEAM❌*" })
}
...
if(!isOwner) {	
    ...
    //await conn.sendMessage(from, { audio: fs.readFileSync("./" + type.ext), mimetype:  originalMessage.message.audioMessage.mimetype, fileName:  `${m.id}.mp3` })	
     const sdata = await conn.sendMessage(delfrom,{sticker: fs.readFileSync("./" + type.ext) ,package: 'DARKLEX-MD 🌟'})
    return await conn.sendMessage(delfrom, { text: `🚫 *This message was deleted !!*\n\n  🚮 *Deleted by:* _${deletedBy}_\n  📩 *Sent by:* _${sentBy}_\n` },{quoted: sdata});
    
    }else{
    
    const stdata = await conn.sendMessage(delfrom,{sticker: fs.readFileSync("./" + type.ext) ,package: 'DARKLEX-MD 🌟'})
    return await conn.sendMessage(delfrom, { text: `🚫 *This message was deleted !!*\n\n  🚮 *Deleted by:* _${deletedBy}_\n  📩 *Sent by:* _${sentBy}_\n` },{quoted: stdata});
...
app.get("/", (req, res) => {
  res.send("📟 DARKLEX-MD Working successfully!");
});
