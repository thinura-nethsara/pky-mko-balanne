...
async(conn, mek, m, {
  ...
  try {
    ...
    let caption = `\`↪️ᴅᴀʀᴋʟᴇx-ᴍᴅ sᴏɴɢ ᴅᴏᴡɴʟᴏᴀᴅᴇʀ↩️\`	
*┌────────────────────┐*
*├ \`🎶 Title\` : ${result.title}*
*├ \`🐼 Views\` : ${result.views}*
...
async (conn, mek, m, {
  ...
  try {
    ...
    let caption = `🎶 *DARKLEX-MD SONG DOWNLOADER* 🎶
...
cmd({
    ...
    try {
        ...
        const caption = `\`🌺DARKLEX-MD FB DOWNLOADER🌺\`\n\n` +
                   `*┌──────────────────*\n` +
                   `*├ \`🐼 Title:\`* ${title}\n` +
                   `*├ \`⏱️ Duration:\`* ${duration}\n` +
                   `*├ \`🔗 Url:\`* ${q}\n` +
 ...
 const response = await axios.get(q, {
  ...
  headers: {
    ...
    await conn.sendMessage(from, {
      video: videoBuffer,
      mimetype: 'video/mp4',
      caption: '✅ *`DARKLEX-MD FB DOWNLOADED SUCSESSFULLY`*'
    }, { quoted: mek });
...
async(conn, mek, m,{from, q, reply}) => {
  try {
    ...
    let listdata = `*\`🗃️ DARKLEX-MD APK DOWNLOADER 🗃️\`*
...
try{            
    ...
    let wm = config.FOOTER
    var result = results.videos[0]
     let caption = `*📹 ᴅᴀʀᴋʟᴇx-ᴍᴅ ᴠɪᴅᴇᴏ ᴅᴏᴡɴʟᴏᴀᴅᴇʀ 📹*
*┌─────────────────────┐*
*├ \`📹 Title\` : ${result.title}* 
*├ \`🐼 Views\` : ${result.views}*
*├ \`⌛Duration\` : ${result.duration}*
...
let listdata = `*\`ᴅᴀʀᴋʟᴇx-ᴍᴅ ᴍᴇᴅɪᴀғɪʀᴇ ᴅᴏᴡʙʟᴏᴀᴅᴇʀ\`*
...
async (conn, mek, m, { from, prefix, l, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    try {
        ...
        let caption = `*\`🎼 ᴅᴀʀᴋʟᴇx sᴘᴛɪғʏ ᴅᴏᴡɴʟᴏᴀᴅᴇʀ 🎼\`*
*┌──────────────────╮*
*├ \`🎶 Title:\`* ${data.title}
*├ \`🧑‍🎤 Artist:\`* ${data.artist}
*├ \`💽 Album:\`* ${data.album}
...
async (conn, mek, m, { from, prefix, q, reply }) => {
    try {
        ...
        const caption = `*\`🎼 DARKLEX-MD SOUNDCLOUD DOWNLOADER 🎼\`*\n\n*🎶 Title:* ${data.title}\n*🔗 URL:* ${q}`;
