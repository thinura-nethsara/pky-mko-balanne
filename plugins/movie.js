...
async (conn, m, mek, { from, q, isMe, isSudo, isOwner, prefix, reply }) => {
    try {
        ...
        const imageUrl = (im && im.trim() && im !== 'undefined') ? im.replace("-150x150", "") : 'https://via.placeholder.com/200x300?text=Movie';
        ...
        // Extract download links – we know they are in 'download_links'
...
async (conn, mek, m, { from, q, isMe, reply }) => {
    ...
    try {
        ...
        let botimg = (datae && datae !== 'undefined' && datae.startsWith('http')) ? datae : 'https://via.placeholder.com/200x300?text=Movie';
        ...
        let thumbnailBuffer;
        try {
            thumbnailBuffer = await (await fetch(botimg)).buffer();
        } catch (e) {
            // If fetch fails, use a default image buffer (optional)
            thumbnailBuffer = await (await fetch('https://via.placeholder.com/200x300?text=Movie')).buffer();
        }
...
async (conn, m, mek, { from, q, isMe, reply }) => {
    try {
        ...
        const finalImgUrl = (imgUrl && imgUrl !== 'undefined') ? imgUrl.replace("-150x150", "") : 'https://via.placeholder.com/200x300?text=Movie';
