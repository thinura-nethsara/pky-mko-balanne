...
// Helper to validate image URL and get buffer
async function getImageBuffer(url) {
    ...
    try {
        ...
        console.warn('Image fetch failed, using fallback:', e.message);
        return FALLBACK_IMAGE_BUFFER;
    }
}
...
async (conn, mek, m, {
    ...
    try {
        ...
        try {
            ...
            imageBuffer = FALLBACK_IMAGE_BUFFER;
        }
...
async (conn, m, mek, { from, q, isMe, isSudo, isOwner, prefix, reply }) => {
    try {
        ...
        // Use fallback if image URL is invalid
        ...
        // Extract download links – use 'download_links'
        ...
        // Prepare image buffer (fallback if not available)
        ...
        normalized.forEach((v) => {
            const buttonData = `${imageUrl || ''}±${v.link}±${movieData.title}\n\n*\`[ ${v.quality} ]\`*`;
            rows.push({
        ...
        // Send as image with buttons (using local buffer)
        await conn.sendMessage(from, {
            image: imageBuffer,
            caption: msg,
            footer: config.FOOTER,
...
async (conn, mek, m, { from, q, isMe, reply }) => {
    ...
    try {
        ...
        if (!datas) {
            return await reply('*Invalid download URL.*');
        }
        ...
        // Get thumbnail buffer (fallback if needed)
...
async (conn, m, mek, { from, q, isMe, reply }) => {
    try {
        ...
        // Get image buffer (fallback if invalid)
        let imageBuffer = FALLBACK_IMAGE_BUFFER;
