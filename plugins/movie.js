...
/**
 * Extract a field from an object using multiple possible keys (including nested).
 * Supports dot notation (e.g., 'imdb.rating') and arrays.
 */
function extractField(obj, keys, fallback = 'N/A') {
    ...
    for (let key of keys) {
        ...
        // Also try simple key (without dots)
    ...
    // If obj is an array itself, try to join
    if (Array.isArray(obj) && obj.length) return obj.join(', ');
...
async (conn, m, mek, { from, q, isMe, isSudo, isOwner, prefix, reply }) => {
    try {
        ...
        console.log('📦 Full Info API Response:', JSON.stringify(movieData, null, 2));

        // Extract fields using multiple possible keys (including nested)
        const title = extractField(movieData, ['title', 'Title', 'movie_title', 'name']);
        const date = extractField(movieData, ['date', 'release_date', 'released', 'year', 'Date', 'Release', 'releaseYear', 'year_of_release']);
        const imdb = extractField(movieData, ['imdb', 'imdb_rating', 'rating', 'imdbRating', 'vote_average', 'IMDb', 'imdb.rating']);
        const runtime = extractField(movieData, ['runtime', 'duration', 'length', 'RunTime', 'run_time', 'movie_length', 'total_time']);
        const subtitle_author = extractField(movieData, ['subtitle_author', 'subtitles_by', 'subtitle_by', 'subtitles_author', 'subtitle_credits', 'subtitle_team']);
        const genres = extractField(movieData, ['genres', 'genre', 'categories', 'Genre', 'category', 'tags', 'type']);
        ...
        let msg = `*☘️ 𝗧ɪᴛʟᴇ ➮* *_${title}_*\n\n` +
                  `*📅 𝗥ᴇʟᴇꜱᴇᴅ ᴅᴀᴛᴇ ➮* _${date}_\n` +
                  `*💃 𝗥ᴀᴛɪɴɢ ➮* _${imdb}_\n` +
                  `*⏰ 𝗥ᴜɴᴛɪᴍᴇ ➮* _${runtime}_\n` +
                  `*💁‍♂️ 𝗦ᴜʙᴛɪᴛʟᴇ ʙʏ ➮* _${subtitle_author}_\n` +
                  `*🎭 𝗚ᴇɴᴀʀᴇꜱ ➮* ${genres}\n`;
        ...
        normalized.forEach(v => {
            const buttonData = `${imageUrl || ''}±${v.link}±${title || 'Movie'}\n\n*\`[ ${v.quality} ]\`*`;
            rows.push({
