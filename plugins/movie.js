...
// Helper to extract a field from an object using multiple possible keys
...
async (conn, m, mek, { from, q, isMe, isSudo, isOwner, prefix, reply }) => {
    try {
        ...
        const date = extractField(movieData, ['date', 'release_date', 'released', 'year', 'Date', 'Release']);
        const imdb = extractField(movieData, ['imdb', 'imdb_rating', 'rating', 'imdbRating', 'vote_average']);
        const runtime = extractField(movieData, ['runtime', 'duration', 'length', 'RunTime']);
        const subtitle_author = extractField(movieData, ['subtitle_author', 'subtitles_by', 'subtitle_by', 'subtitles_author']);
        const genres = extractField(movieData, ['genres', 'genre', 'categories', 'Genre']);
